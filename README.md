...
https://github.com/rrthomas/Sokoban  

by Reuben Thomas <rrt@sc3d.org>  

Sokoban is a simple puzzle game in which you tunnel through caverns
...
I originally wrote Sokoban for Acorn RISC OS. Original and updated [RISC OS
versions](<https://github.com/rrthomas/Sokoban/RISC OS.md>) are available.
...
[latest release](https://github.com/Sokoban/releases):
...
* The GNU/Linux version is a single binary; you need to make it executable
  ...
  `LD_PRELOAD=/usr/lib/x86_64-linux-gnu/libstdc++.so.6 ./Sokoban-Linux-X64`.
...
If you are a Python user, this is the simplest way to get Sokoban on most
machines.

Install with `pip`: `pip install Sokoban`, then execute the command
`Sokoban`.
...
Sokoban from GitHub. (If this doesn’t make sense to you, sorry! I hope to
provide a simpler way to edit and play new levels soon.)

The level files are in the `Sokoban/levels` subdirectory of the project, and
...
`pip install .` or run it directly with `PYTHONPATH=. python -m Sokoban`.
...
+ A complete level set requires a Tiled tileset. You can simply copy the
  tileset file `Sokoban.tsx` from `Sokoban/levels`.
...
I welcome [pull requests](https://github.com/rrthomas/Sokoban/pulls) for new
...
welcome. See the [open issues](https://github.com/rrthomas/Sokoban/) for
...
Sokoban is distributed under the GNU Public License version 3, or, at your
option, any later version. See the file COPYING.

THIS PROGRAM IS PROVIDED AS IS, WITH NO WARRANTY. USE IS AT THE USER'S RISK.
Sokoban’s code is copyright Reuben Thomas, and its levels and graphics by
...
system font, as used on the Acorn Archimedes on which Sokoban was originally
